...
public class ContactFragment extends BaseFragment {
    ...
    private void initViews(View view) {
        ...
        mContactLayout.getTitleBar().mLeftIcon.setVisibility(View.VISIBLE);
        mContactLayout.getTitleBar().mLeftIcon.setClickable(true);
        mContactLayout.getContactListView().setOnItemClickListener(new ContactListView.OnItemClickListener() {
            @Override
            public void onItemClick(int position, ContactItemBean contact) {
                if (position == 0) {
                    Intent intent = new Intent(ChatApplication.instance(), NewFriendActivity.class);
                    intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                    ChatApplication.instance().startActivity(intent);
                } else if (position == 1) {
                    Intent intent = new Intent(ChatApplication.instance(), GroupListActivity.class);
                    intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                    ChatApplication.instance().startActivity(intent);
                } else if (position == 2) {
                    Intent intent = new Intent(ChatApplication.instance(), BlackListActivity.class);
                    intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                    ChatApplication.instance().startActivity(intent);
                } else {
                    Intent intent = new Intent(ChatApplication.instance(), FriendProfileActivity.class);
                    intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
