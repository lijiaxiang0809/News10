        ...
        public void onDraw(ICustomMessageViewGroup parent, MessageInfo info) {
            ...
            switch(customMessageData.type) {
                case CustomMessageData.TYPE_HYPERLINK:
                    view = LayoutInflater.from(ChatApplication.instance()).inflate(R.layout.test_custom_message_layout1, null, false);
                    // 把自定义消息view添加到TUIKit内部的父容器里
                    parent.addMessageContentView(view);
                    break;
                case CustomMessageData.TYPE_PUSH_TEXT_VIDEO:
                    view = LayoutInflater.from(ChatApplication.instance()).inflate(R.layout.test_custom_message_layout2, null, false);
                    // 把自定义消息view添加到TUIKit内部的父容器里
