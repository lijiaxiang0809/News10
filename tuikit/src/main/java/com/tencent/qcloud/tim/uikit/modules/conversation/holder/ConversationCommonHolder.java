...
//        conversationIconView.setIconUrls(null); // 如果自己要设置url，这行代码需要删除
        if (conversation.isGroup()) {
            ...
            String userIndentify = conversation.getId();
            TIMUserProfile profile = TIMFriendshipManager.getInstance().queryUserProfile(userIndentify);
            if (profile != null && !TextUtils.isEmpty(profile.getFaceUrl())){
                conversationIconView.setNetUserIcon(profile.getFaceUrl());
            }
        }
