    ...
    public void onBindViewHolder(final ContactAdapter.ViewHolder holder, final int position) {
        ...
        if (contactBean.isGroup()) {
            ...
            if (!TextUtils.isEmpty(contactBean.getId())) {
                TIMUserProfile profile = TIMFriendshipManager.getInstance().queryUserProfile(contactBean.getId());
                if (profile != null && !TextUtils.isEmpty(profile.getFaceUrl())) {
                    GlideEngine.loadImage(holder.avatar, profile.getFaceUrl());
                } else holder.avatar.setImageResource(R.drawable.ic_personal_member);
            } else
